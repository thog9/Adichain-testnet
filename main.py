...
░░░╚═╝░░░╚═╝░░╚═╝░╚════╝░░╚═════╝░░░░╚═╝░░░░╚════╝░░╚════╝░╚══════╝╚═╝░░╚═╝░╚═════╝░╚═════╝░
    ...
    print_border("ADI PREDICTSTREET", Fore.GREEN)
...
async def run_quest(language: str):
    from scripts.quest import run_quest
    await run_quest(language)
...
async def run_pack(language: str):
    from scripts.pack import run_pack
    await run_pack(language)

async def run_connect_wallet(language: str):
    from scripts.connect_wallet import run_connect_wallet
    await run_connect_wallet(language)

async def run_mint(language: str):
    from scripts.mint import run_mint
    await run_mint(language)
...
SCRIPT_MAP = {
    ...
    "connect": run_connect,
    "pack": run_pack,
    "connect_wallet": run_connect_wallet,
    "mint": run_mint,
    "exit": cmd_exit
}
...
def get_available_scripts(language):
    scripts = {
        'vi': [
            {"name": "1. Kết nối Twitter (X OAuth)", "value": "connect"},
            {"name": "2. Điểm danh hằng ngày", "value": "checkin"},
            {"name": "3. Tự động làm nhiệm vụ", "value": "quest"},
            {"name": "4. Mở pack hàng ngày", "value": "pack"},
            {"name": "5. Kết nối Ví (Multi-chain)", "value": "connect_wallet"},
            {"name": "6. Mint ADI Card", "value": "mint"},
